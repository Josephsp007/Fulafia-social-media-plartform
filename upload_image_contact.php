...
if(count($_FILES["file"]["name"]) > 0)
{
 ...
 for($count=0; $count<count($_FILES["file"]["name"]); $count++)
 {
  ...
  $location = 'feedback-images/' . $file_name;
  if(move_uploaded_file($tmp_name, $location))
  {
   $contacted = time();
   $title = "Image Attachment";
   $query = "INSERT INTO feedback (attachment, userid, contacted, title) VALUES (:file_name, :userid, :contacted, :title)";
   $statement = $pdo->prepare($query);
   $statement->bindParam(":file_name", $file_name, PDO::PARAM_STR);
   $statement->bindParam(":userid", $userid, PDO::PARAM_STR);
   $statement->bindParam(":contacted", $contacted, PDO::PARAM_STR);
